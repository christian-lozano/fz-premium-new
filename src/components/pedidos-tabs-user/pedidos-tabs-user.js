...
export default function PedidosTabsUser() {
  ...
  useEffect(() => {
    if (session?.user.id) {
      client
        .fetch(
          groq`*[_type == "pedidos" && estado != "pendiente" && userId match "${session?.user.id}"]`
        )
        .then((result) => {
                  ...
                  {el.tipoEntrega === "envio" && (
                    <>
                      ...
                      >
                        ...
                        <span className="ml-10">Preparación para entregar</span>
                      </div>
                      ...
                      >
                        ...
                        <span className="ml-10">En Camino a ser entregado</span>
                      </div>
                    </>
                  )}
                  {/*  */}
